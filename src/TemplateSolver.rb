...
class TemplateSolver < Solver
  ...
  def solve()
    until @board.solved?
      ...
      base = generate_base
      template = generate_template(base)
      template = fix_overlapped_same_numbers(template)
      template = delete_overlapped_differnt_numbers(template)
      numbers = generate_numbers(template)
  ...
  def generate_template(base)
    ...
    for i in @board.n_min..@board.n_max do
      ...
      until flag
        ...
        idx += 1
        # temp.sizeが@board.x_size以上の場合、tempをtemplateに追加し、探索を継続
  ...
  def fix_overlapped_same_numbers(template)
    t = Hash.new
    for i in @board.n_min..@board.n_max do
  ...
  def delete_overlapped_differnt_numbers(template)
